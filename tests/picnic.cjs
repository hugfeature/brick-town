...
(async()=>{
 ...
 await p.keyboard.down('ArrowRight');await advance(1000);await p.keyboard.up('ArrowRight');await advance(120000);assert(await p.locator('#continuePicnic').isVisible());assert((await p.locator('#veil').innerText()).includes('已经找到 2'));
 await p.locator('#continuePicnic').click();assert((await p.locator('#missionHud').innerText()).includes('2/3'));await advance(30000);assert.equal(await p.locator('[data-reward]').count(),3);assert((await p.locator('#missionHud').innerText()).includes('3/3'));assert.equal((await saved()).owned.length,0);
